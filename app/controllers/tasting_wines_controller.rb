class TastingWinesController < ApplicationController
  ...
  def reveal
    if @tasting_wine.tasting.is_completed?
      ...
      if @tasting_wine.save
        if @tasting_wine.tasting.is_completed?
          flash[:notice] = "#{@tasting_wine.wine.name} revealed as wine #{@tasting_wine.wine_number}. Tasting Complete!"
          redirect_to tasting_path(@tasting_wine.tasting)
        else
  ...
  def create
    @tasting_wine = TastingWine.new(create_params)
    if @tasting_wine.tasting.is_closed?
      ...
      @tasting_wine.tasting.guests.each do |g|
        WineReview.create_next_in_sequence_for_guest(@tasting_wine.tasting, g.taster_id)
      end
